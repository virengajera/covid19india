...
import { lateststatefetch, statetocodename, statenametostatecode } from './fetchlatest_statejson.js'
...
function updatemaphoverdata(d, i, maptype) {
    ...
    if (maptype == "state") {
        a1.text("State : " + i.properties.st_nm)
        ...
        if (i.properties.totalcasedata.st_data) {
            a2.text("Total Confirmed : " + i.properties.totalcasedata.st_data.confirmcase)
            a3.text("Total Active : " + i.properties.totalcasedata.st_data.activecase)
            a4.text("Total Recovered : " + i.properties.totalcasedata.st_data.recovercase)
            a5.text("Total Death : " + i.properties.totalcasedata.st_data.deathcase)
        }
        else {
        ...
        b1.text("District : " + i.properties.district)
        b2.text("Total Confirmed : " + i.properties.totalcasedata.confirmcase)
        b3.text("Total Active : " + i.properties.totalcasedata.activecase)
        b4.text("Total Recovered : " + i.properties.totalcasedata.recovercase)
        b5.text("Total Death : " + i.properties.totalcasedata.deathcase)
    }
    else {
...
async function chart_data(st_code,case_btn_cty){

    const seriresdata = await state_timeseries(st_code)

 
    chart_plotter(seriresdata,case_btn_cty)
    chart_button_toggle(seriresdata)
}

chart_data("TT")

async function chart_plotter(seriresdata,case_btn_category="total") {

    if (case_btn_category == 'total') {
        plotchart('confirm-chart', 'confirmed', seriresdata, case_btn_category)
        plotchart('active-chart', 'active', seriresdata, case_btn_category)
        plotchart('recover-chart', 'recovered', seriresdata, case_btn_category)
        plotchart('death-chart', 'deceased', seriresdata, case_btn_category)
    }
    else {
        lollipopchart('confirm-chart', 'confirmed', seriresdata, case_btn_category)
        lollipopchart('active-chart', 'active', seriresdata, case_btn_category)
        lollipopchart('recover-chart', 'recovered', seriresdata, case_btn_category)
        lollipopchart('death-chart', 'deceased', seriresdata, case_btn_category)
    }
}
...
function dropdownchange(d) {
    ...
    if (btn9.classed('active')) {
        ...
        chart_data(change_cd,"total")
    }
    else {
        
        chart_data(change_cd,"delta")
        
    }

}

function chart_button_toggle_check(a,seriresdata) {
    d3.selectAll('.firstg').remove()
    const nwbtnarry = a.filter(d => d.classed('active'))
    const nwbtnarry2 = nwbtnarry.map(d => d.attr('value'))
    if (nwbtnarry2.includes('total')) {
        chart_plotter(seriresdata,"total")

    }
    else {
        
        chart_plotter(seriresdata,"delta")
...
function chart_button_toggle(seriresdata) {
    ...
    btn9.on('click', () => {
        if (btn9.classed('active')) {
            ...
            chart_button_toggle_check(new Array(btn9, btn10),seriresdata)
        }
    })
    btn10.on('click', () => {
